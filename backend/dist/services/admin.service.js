...
exports.listWithdrawals = exports.updateWithdrawalStatus = exports.initiateWithdrawal = exports.calculateWithdrawalFees = exports.MARVIANS_WITHDRAWAL_FEE_RATE = exports.repairSponsorsConfigIfNeeded = exports.getSponsorsConfig = exports.saveSponsorsConfig = exports.updateSiteConfig = exports.listCandidates = exports.getDashboardStats = void 0;
...
const sponsorsConfig_1 = require("../utils/sponsorsConfig");
// =============================================================================
...
const getDashboardStats = async () => {
    // 1. Total Votes
    const totalVotes = await prisma_1.default.vote.count({
        where: { status: 'SUCCESS' }
    });
    // 2. Total Candidates
    const totalCandidates = await prisma_1.default.candidate.count();
    // 3. Total Revenue
    const revenueResult = await prisma_1.default.vote.aggregate({
        where: { status: 'SUCCESS' },
        _sum: { amount: true }
    });
    const totalRevenue = revenueResult._sum.amount || 0;
    // 4. Pending Withdrawals
    const pendingWithdrawals = await prisma_1.default.withdrawal.count({
        where: { status: 'PENDING' }
    });
    // 5. Recent Candidates (last 5)
    const recentCandidates = await prisma_1.default.candidate.findMany({
    ...
    const recentVotes = await prisma_1.default.vote.findMany({
        orderBy: { createdAt: 'desc' },
        take: 10,
        select: {
            id: true,
    ...
    const categoriesStats = await prisma_1.default.category.findMany({
        select: {
            name: true,
 ...
 */
const updateSiteConfig = async (configs) => {
    ...
    const operations = configs.map(config => prisma_1.default.siteConfiguration.upsert({
    ...
    return await prisma_1.default.$transaction(operations);
};
 ...
 */
const saveSponsorsConfig = async (sponsors, replaceAll = true) => {
    const existing = await prisma_1.default.siteConfiguration.findUnique({ where: { configKey: 'sponsors' } });
    const existingList = (0, sponsorsConfig_1.parseSponsorsConfig)(existing?.configValue);
    const merged = (0, sponsorsConfig_1.mergeSponsorsConfig)(existingList, sponsors, replaceAll);
    await (0, exports.updateSiteConfig)([{ key: 'sponsors', value: JSON.stringify(merged) }]);
    return merged;
};
exports.saveSponsorsConfig = saveSponsorsConfig;
/** Récupère les sponsors depuis la configuration site (liste résolue avec défauts si incomplète) */
const getSponsorsConfig = async () => {
    const row = await prisma_1.default.siteConfiguration.findUnique({ where: { configKey: 'sponsors' } });
    const fromDb = (0, sponsorsConfig_1.parseSponsorsConfig)(row?.configValue);
    return (0, sponsorsConfig_1.resolvePublicSponsors)(fromDb);
};
exports.getSponsorsConfig = getSponsorsConfig;
/** Répare la config sponsors en DB si la liste est incomplète */
const repairSponsorsConfigIfNeeded = async () => {
    const row = await prisma_1.default.siteConfiguration.findUnique({ where: { configKey: 'sponsors' } });
    const fromDb = (0, sponsorsConfig_1.parseSponsorsConfig)(row?.configValue);
    const resolved = (0, sponsorsConfig_1.resolvePublicSponsors)(fromDb);
    const resolvedJson = JSON.stringify(resolved);
    if (!row?.configValue || row.configValue !== resolvedJson) {
        await (0, exports.updateSiteConfig)([{ key: 'sponsors', value: resolvedJson }]);
    }
    return resolved;
};
exports.repairSponsorsConfigIfNeeded = repairSponsorsConfigIfNeeded;
// Commission Marvians (Smobilpay) prélevée sur chaque retrait en fin de concours
exports.MARVIANS_WITHDRAWAL_FEE_RATE = 0.03;
/**
 ...
 */
const calculateWithdrawalFees = (requestedAmount) => {
    const feeAmount = Math.floor(requestedAmount * exports.MARVIANS_WITHDRAWAL_FEE_RATE);
    const netAmount = requestedAmount - feeAmount;
    return { feeAmount, netAmount, feeRate: exports.MARVIANS_WITHDRAWAL_FEE_RATE };
};
exports.calculateWithdrawalFees = calculateWithdrawalFees;
// =============================================================================
 ...
 */
const initiateWithdrawal = async (requestedAmount) => {
    // Validation du montant : doit être strictement positif
    if (requestedAmount <= 0) {
        throw new AppError_1.AppError('Le montant doit être supérieur à 0.', 400);
    }
    // Calcul des frais et du montant net via la fonction utilitaire dédiée
    const { feeAmount, netAmount } = (0, exports.calculateWithdrawalFees)(requestedAmount);
...
const updateWithdrawalStatus = async (withdrawalId) => {
    ...
    if (withdrawal.status !== 'PENDING') {
        throw new AppError_1.AppError('Seuls les retraits en statut PENDING peuvent être marqués comme COMPLETED.', 400);
    }
    // Mise à jour du statut vers COMPLETED
    const updatedWithdrawal = await prisma_1.default.withdrawal.update({
        where: { id: withdrawalId },
